-- flappy.cc - FULL SCRIPT (ScriptHub MASSIVELY EXPANDED - 15+ FE Trolling + 10 New Games) local Rayfield = loadstring(game:HttpGet('https://sirius.menu/rayfield', true))()

local Window = Rayfield:CreateWindow({ Name = "flappy.cc", LoadingTitle = "flappy.cc", LoadingSubtitle = "flappy is not a skid w dev | Xeno", ConfigurationSaving = { Enabled = true, FolderName = "flappy.cc", FileName = "FullConfig" } })

Rayfield:Notify({Title = "flappy.cc", Content = "ScriptHub MASSIVELY EXPANDED - 15+ FE Trolling + 10 New Games", Duration = 6})

local Camera = workspace.CurrentCamera local UserInput = game:GetService("UserInputService") local RunService = game:GetService("RunService") local VirtualUser = game:GetService("VirtualUser")

-- ====================== COMBAT TAB ====================== local Combat = Window:CreateTab("Combat", 4483362458)

-- Settings local AimbotEnabled = false local SilentAimEnabled = false local VisCheck = true local TargetTeammates = false local TargetNPCs = false local BulletPrediction = false local BulletDropCompensation = true local Smoothness = 12 local AimFOV = 120 local MaxAimDistance = 300 local AimPart = "Head" local BulletSpeed = 1500 local GameFOV = 70

local currentTarget = nil local lockedTarget = nil

local AimbotKey = Enum.UserInputType.MouseButton2 -- Default RMB local SilentAimKey = Enum.KeyCode.R -- Default R

local Camera = workspace.CurrentCamera local RunService = game:GetService("RunService") local UserInput = game:GetService("UserInputService")

-- FOV Circle local FOVCircle = Drawing.new("Circle") FOVCircle.Thickness = 2 FOVCircle.NumSides = 100 FOVCircle.Color = Color3.fromRGB(0, 255, 255) FOVCircle.Transparency = 0.75 FOVCircle.Filled = false FOVCircle.Visible = false

-- ====================== UI ====================== Combat:CreateToggle({ Name = "AI Aimbot", CurrentValue = false, Callback = function(v) AimbotEnabled = v end })

Combat:CreateKeybind({ Name = "Aimbot Keybind", CurrentKeybind = "MouseButton2", HoldToUse = true, Callback = function() end })

Combat:CreateToggle({ Name = "Silent Aim", CurrentValue = false, Callback = function(v) SilentAimEnabled = v end })

Combat:CreateKeybind({ Name = "Silent Aim Keybind", CurrentKeybind = "R", HoldToUse = true, Callback = function() end })

Combat:CreateToggle({Name = "VisCheck (Only Visible Players)", CurrentValue = true, Callback = function(v) VisCheck = v end}) Combat:CreateToggle({Name = "Target Teammates", CurrentValue = false, Callback = function(v) TargetTeammates = v end}) Combat:CreateToggle({Name = "Target NPCs/Bots", CurrentValue = false, Callback = function(v) TargetNPCs = v end}) Combat:CreateToggle({Name = "Bullet Prediction", CurrentValue = false, Callback = function(v) BulletPrediction = v end}) Combat:CreateToggle({Name = "Bullet Drop Compensation", CurrentValue = true, Callback = function(v) BulletDropCompensation = v end})

Combat:CreateInput({Name = "Max Aim Distance (studs)", PlaceholderText = "300", RemoveTextAfterFocusLost = false, Callback = function(text) local num = tonumber(text) if num then MaxAimDistance = math.clamp(num, 50, 2000) end end}) Combat:CreateInput({Name = "Aimbot Smoothness (1-25)", PlaceholderText = "12", RemoveTextAfterFocusLost = false, Callback = function(text) local num = tonumber(text) if num then Smoothness = math.clamp(num, 1, 25) end end}) Combat:CreateInput({Name = "Aimbot FOV (30-400)", PlaceholderText = "120", RemoveTextAfterFocusLost = false, Callback = function(text) local num = tonumber(text) if num then AimFOV = math.clamp(num, 30, 400) end end}) Combat:CreateInput({Name = "Bullet Speed (studs/s)", PlaceholderText = "1500", RemoveTextAfterFocusLost = false, Callback = function(text) local num = tonumber(text) if num then BulletSpeed = math.clamp(num, 500, 5000) end end})

Combat:CreateDropdown({Name = "Aim Part", Options = {"Head", "UpperTorso", "HumanoidRootPart"}, CurrentOption = {"Head"}, Callback = function(opt) AimPart = opt[1] end}) Combat:CreateToggle({Name = "Show FOV Circle", CurrentValue = false, Callback = function(v) FOVCircle.Visible = v end})

-- Game FOV Changer Combat:CreateInput({ Name = "Game FOV (30-120)", PlaceholderText = "70", RemoveTextAfterFocusLost = false, Callback = function(text) local num = tonumber(text) if num then GameFOV = math.clamp(num, 30, 120) Camera.FieldOfView = GameFOV end end })

-- ====================== TARGET SELECTION ====================== task.spawn(function() while true do task.wait(0.03)
...
-- ====================== AIMING ====================== RunService.RenderStepped:Connect(function() FOVCircle.Position = Vector2.new(Camera.ViewportSize.X/2, Camera.ViewportSize.Y/2) FOVCircle.Radius = AimFOV
...
print("✅ Combat Tab with Custom Keybinds Loaded!") -- ====================== TROLL TAB ====================== local Troll = Window:CreateTab("Troll", 4483362458)

-- Settings local KillAllEnabled = false local KillAllLoop = false local KillTeammates = false local KillSelectedEnabled = false local LoopKillSelected = false

local SelectedPlayer = nil local killedPlayers = {} -- For one-time Kill All local trollTarget = nil

local Camera = workspace.CurrentCamera local RunService = game:GetService("RunService") local UserInput = game:GetService("UserInputService") local VirtualUser = game:GetService("VirtualUser")

-- ====================== UI ====================== Troll:CreateToggle({ Name = "Kill All (Enemies Only)", CurrentValue = false, Callback = function(v) KillAllEnabled = v end })

Troll:CreateToggle({ Name = "Kill All Loop", CurrentValue = false, Callback = function(v) KillAllLoop = v end })

Troll:CreateToggle({ Name = "Kill Teammates", CurrentValue = false, Callback = function(v) KillTeammates = v end })

-- Player Dropdown (Auto-updates) local playerList = {} for _, plr in ipairs(game.Players:GetPlayers()) do if plr ~= game.Players.LocalPlayer then table.insert(playerList, plr.Name) end end

Troll:CreateDropdown({ Name = "Select Player", Options = playerList, CurrentOption = {""}, Callback = function(opt) local plrName = opt[1] if plrName and plrName ~= "" then local plr = game.Players:FindFirstChild(plrName) if plr and plr.Character then SelectedPlayer = plr.Character:FindFirstChild("HumanoidRootPart") end else SelectedPlayer = nil end end })
...
-- ====================== MAIN TROLL LOOP ====================== RunService.RenderStepped:Connect(function() trollTarget = nil
...
-- Auto refresh dropdown when players join/leave game.Players.PlayerAdded:Connect(function() -- You can refresh dropdown if needed, but Rayfield usually handles it end)

print("✅ Troll Tab Loaded - TP Behind + Team Control") -- ===================== CHAOS CHEATING SIMULATOR ESP ===================== local Players = game:GetService("Players") local RunService = game:GetService("RunService")

local LocalPlayer = Players.LocalPlayer local Camera = workspace.CurrentCamera

local ESPTab = Window:CreateTab("ESP", 4483362458)

-- SETTINGS local ESPEnabled = false local ShowTeammates = true

local ShowBoxes = true local ShowTracers = true local ShowNames = true local ShowHealth = true local ShowDistance = true local Chams = false

local MaxDistance = 1000

local ESPCache = {}

-- SCREEN GUI FOR TRACERS local TracerFolder = Instance.new("ScreenGui") TracerFolder.Name = "CHAOS_TRACERS" TracerFolder.ResetOnSpawn = false TracerFolder.Parent = game.CoreGui

-- CLEAN local function Clear(plr) local data = ESPCache[plr] if not data then return end
...
-- CREATE local function Create(plr) if ESPCache[plr] then return end
...
-- LOOP RunService.RenderStepped:Connect(function() if not ESPEnabled then return end
...
for _, plr in ipairs(Players:GetPlayers()) do
    ...
    esp.label.Text =
        (ShowNames and plr.Name or "") ..
        (ShowHealth and " | HP: " .. math.floor(hum.Health) or "") ..
        (ShowDistance and string.format(" | %.0fm", dist) or "")
    ...
    -- SIMPLE TRACER (screen line)
    if ShowTracers then
        esp.tracer.Visible = true
        esp.tracer.Position = UDim2.new(0, Camera.ViewportSize.X/2, 0, Camera.ViewportSize.Y)
        esp.tracer.Size = UDim2.new(0, pos.X - Camera.ViewportSize.X/2, 0, pos.Y - Camera.ViewportSize.Y)
...
-- ===================== UI ===================== ESPTab:CreateToggle({ Name = "Enable ESP", CurrentValue = false, Callback = function(v) ESPEnabled = v if not v then for plr in pairs(ESPCache) do Clear(plr) end end end })

ESPTab:CreateToggle({ Name = "Show Teammates", CurrentValue = true, Callback = function(v) ShowTeammates = v end })

ESPTab:CreateToggle({ Name = "Names", CurrentValue = true, Callback = function(v) ShowNames = v end })

ESPTab:CreateToggle({ Name = "Health", CurrentValue = true, Callback = function(v) ShowHealth = v end })

ESPTab:CreateToggle({ Name = "Distance", CurrentValue = true, Callback = function(v) ShowDistance = v end })

ESPTab:CreateToggle({ Name = "Tracers", CurrentValue = true, Callback = function(v) ShowTracers = v end })

ESPTab:CreateToggle({ Name = "Chams", CurrentValue = false, Callback = function(v) Chams = v end })

ESPTab:CreateSlider({ Name = "Max Distance", Range = {50, 3000}, Increment = 50, CurrentValue = 1000, Callback = function(v) MaxDistance = v end }) -- ====================== MOVEMENT ====================== local Movement = Window:CreateTab("Movement", 4483362458)

-- Settings local WalkSpeedEnabled = false local JumpPowerEnabled = false local InfiniteJumpEnabled = false local NoClipEnabled = false local FlyEnabled = false local BhopEnabled = false local SpeedMultiplierEnabled = false

local CustomWalkSpeed = 50 local CustomJumpPower = 50 local FlySpeed = 50 local SpeedMultiplier = 1.5

local FlyConnection = nil local NoClipConnection = nil

local lp = game.Players.LocalPlayer local Camera = workspace.CurrentCamera

-- Apply WalkSpeed & JumpPower local function ApplyMovement() local char = lp.Character if not char then return end local hum = char:FindFirstChild("Humanoid") if hum then if WalkSpeedEnabled then hum.WalkSpeed = CustomWalkSpeed end if JumpPowerEnabled then hum.JumpPower = CustomJumpPower end end end

-- ====================== INFINITE JUMP ====================== game:GetService("UserInputService").JumpRequest:Connect(function() if InfiniteJumpEnabled then local hum = lp.Character and lp.Character:FindFirstChild("Humanoid") if hum then hum:ChangeState(Enum.HumanoidStateType.Jumping) end end end)

-- ====================== BHOP ====================== game:GetService("RunService").Heartbeat:Connect(function() if not BhopEnabled then return end local hum = lp.Character and lp.Character:FindFirstChild("Humanoid") if hum and game:GetService("UserInputService"):IsKeyDown(Enum.KeyCode.Space) then hum:ChangeState(Enum.HumanoidStateType.Jumping) end end)

-- ====================== NOCLIP ====================== local function StartNoClip() if NoClipConnection then return end NoClipConnection = game:GetService("RunService").Stepped:Connect(function() if not NoClipEnabled then return end local char = lp.Character if char then for _, part in pairs(char:GetDescendants()) do if part:IsA("BasePart") then part.CanCollide = false end end end end) end

local function StopNoClip() if NoClipConnection then NoClipConnection:Disconnect() NoClipConnection = nil end local char = lp.Character if char then for _, part in pairs(char:GetDescendants()) do if part:IsA("BasePart") and part.Name ~= "HumanoidRootPart" then part.CanCollide = true end end end end

-- ====================== FLY ====================== local function StartFly() if FlyConnection then return end
...
local function StopFly() if FlyConnection then FlyConnection:Disconnect() FlyConnection = nil end local char = lp.Character if char and char:FindFirstChild("HumanoidRootPart") then local root = char.HumanoidRootPart for _, v in pairs(root:GetChildren()) do if v:IsA("BodyVelocity") or v:IsA("BodyGyro") then v:Destroy() end end end end

-- ====================== SPEED MULTIPLIER ====================== game:GetService("RunService").Heartbeat:Connect(function() if not SpeedMultiplierEnabled then return end local char = lp.Character local hum = char and char:FindFirstChild("Humanoid") if hum then hum.WalkSpeed = CustomWalkSpeed * SpeedMultiplier end end)

-- ====================== UI ====================== Movement:CreateToggle({ Name = "Enable WalkSpeed", CurrentValue = false, Callback = function(v) WalkSpeedEnabled = v; ApplyMovement() end })

Movement:CreateInput({ Name = "WalkSpeed Value", PlaceholderText = "50", RemoveTextAfterFocusLost = false, Callback = function(text) local num = tonumber(text) if num then CustomWalkSpeed = math.clamp(num, 0, 1000) if WalkSpeedEnabled then ApplyMovement() end end end })

Movement:CreateToggle({ Name = "Speed Multiplier", CurrentValue = false, Callback = function(v) SpeedMultiplierEnabled = v end })

Movement:CreateInput({ Name = "Speed Multiplier (x)", PlaceholderText = "1.5", RemoveTextAfterFocusLost = false, Callback = function(text) local num = tonumber(text) if num then SpeedMultiplier = math.clamp(num, 0.5, 10) end end })

Movement:CreateToggle({ Name = "Enable JumpPower", CurrentValue = false, Callback = function(v) JumpPowerEnabled = v; ApplyMovement() end })

Movement:CreateInput({ Name = "JumpPower Value", PlaceholderText = "50", RemoveTextAfterFocusLost = false, Callback = function(text) local num = tonumber(text) if num then CustomJumpPower = math.clamp(num, 0, 1000) if JumpPowerEnabled then ApplyMovement() end end end })

Movement:CreateToggle({ Name = "Infinite Jump", CurrentValue = false, Callback = function(v) InfiniteJumpEnabled = v end })

Movement:CreateToggle({ Name = "Bhop (Hold Space)", CurrentValue = false, Callback = function(v) BhopEnabled = v end })

Movement:CreateToggle({ Name = "NoClip", CurrentValue = false, Callback = function(v) NoClipEnabled = v if v then StartNoClip() else StopNoClip() end end })

Movement:CreateToggle({ Name = "Fly (WASD + E/Q)", CurrentValue = false, Callback = function(v) FlyEnabled = v if v then StartFly() else StopFly() end end })

Movement:CreateInput({ Name = "Fly Speed", PlaceholderText = "50", RemoveTextAfterFocusLost = false, Callback = function(text) local num = tonumber(text) if num then FlySpeed = math.clamp(num, 10, 500) end end })

-- Respawn Handler lp.CharacterAdded:Connect(function() task.wait(0.6) ApplyMovement() end)

print("✅ Full Movement Tab Loaded (Speed Multiplier + Bhop Added)") -- ====================== CUSTOM CROSSHAIR TAB ====================== local CrosshairTab = Window:CreateTab("Crosshair", 4483362458)

local DavidStarEnabled = false local StarSize = 35 local StarColor = Color3.fromRGB(0, 255, 255)

local StarObjects = {} local CrosshairConnection = nil local Camera = workspace.CurrentCamera local RunService = game:GetService("RunService")

-- Create David Star local function CreateDavidStar() -- Full clear for _, obj in pairs(StarObjects) do if obj and obj.Remove then obj:Remove() end end table.clear(StarObjects)
...
local function StartDavidStar() if CrosshairConnection then return end
...
local function StopDavidStar() DavidStarEnabled = false -- Force flag off
...
-- ====================== UI ====================== CrosshairTab:CreateToggle({ Name = "David Star Crosshair", CurrentValue = false, Callback = function(v) DavidStarEnabled = v if v then StartDavidStar() else StopDavidStar() end end })

CrosshairTab:CreateSlider({ Name = "Star Size", Range = {15, 60}, Increment = 1, CurrentValue = 35, Callback = function(v) StarSize = v if DavidStarEnabled then CreateDavidStar() end end })

CrosshairTab:CreateColorPicker({ Name = "Star Color", Color = Color3.fromRGB(0, 255, 255), Callback = function(v) StarColor = v if DavidStarEnabled then CreateDavidStar() end end })

CrosshairTab:CreateToggle({ Name = "Hide In-Game GUI (Clean Screen)", CurrentValue = false, Callback = function(v) if v then for _, gui in ipairs(game.Players.LocalPlayer.PlayerGui:GetChildren()) do if gui:IsA("ScreenGui") and not gui.Name:find("Rayfield") then gui.Enabled = false end end else for _, gui in ipairs(game.Players.LocalPlayer.PlayerGui:GetChildren()) do if gui:IsA("ScreenGui") then gui.Enabled = true end end end end })

CrosshairTab:CreateButton({ Name = "Force Refresh Crosshair", Callback = function() if DavidStarEnabled then StopDavidStar() task.wait(0.08) StartDavidStar() end end })
...
-- ================================ -- UNIVERSAL SCRIPTS (Work on ANY Game) -- ================================ Hub:CreateSection("🌐 UNIVERSAL (Works on ANY Game)")
...
-- ================================ -- TROLLING / FE SCRIPTS -- ================================ Hub:CreateSection("🤡 TROLLING (FE Scripts)")
...
-- ================================ -- BROOKHAVEN RP -- ================================ Hub:CreateSection("🏡 BROOKHAVEN RP")
...
-- ================================ -- DA HOOD -- ================================ Hub:CreateSection("🔫 DA HOOD")
...
-- ================================ -- CALI SHOOTOUT -- ================================ Hub:CreateSection("🔥 CALI SHOOTOUT")
...
-- ================================ -- THA BRONX 3 -- ================================ Hub:CreateSection("🐍 THA BRONX 3")
...
-- ================================ -- SHOTTAS OF MIAMI V2 -- ================================ Hub:CreateSection("🔥 SHOTTAS OF MIAMI V2")
...
-- ================================ -- RIVALS -- ================================ Hub:CreateSection("⚔️ RIVALS")
...
-- ================================ -- PRISON LIFE / REDWOOD PRISON -- ================================ Hub:CreateSection("🚔 PRISON LIFE")
...
-- ================================ -- ARSENAL / PHANTOM FORCES -- ================================ Hub:CreateSection("🎯 ARSENAL")
...
-- ================================ -- MURDER MYSTERY 2 -- ================================ Hub:CreateSection("🔪 MURDER MYSTERY 2")
...
Hub:CreateButton({Name = "MM2 Auto Win (Sheriff, Innocent, Murderer) - Mobile/PC", Callback = function() loadstring(game:HttpGet("https://raw.githubusercontent.com/Drifter0507/GUIS/main/MURDER%20MYSTERY%202", true))() end}) -- ================================ -- BLADE BALL -- ================================ Hub:CreateSection("⚔️ BLADE BALL")
...
-- ================================ -- BEDWARS -- ================================ Hub:CreateSection("🛡️ BEDWARS")
...
-- ================================ -- JAILBREAK -- ================================ Hub:CreateSection("🚗 JAILBREAK")
...
-- ================================ -- TOWER OF HELL -- ================================ Hub:CreateSection("🗼 TOWER OF HELL")
...
-- ================================ -- NATURAL DISASTER SURVIVAL (UPDATED) -- ================================ Hub:CreateSection("🌋 NATURAL DISASTER SURVIVAL")
...
-- ================================ -- FLOOD ESCAPE 2 -- ================================ Hub:CreateSection("🌊 FLOOD ESCAPE 2")
